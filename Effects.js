...
(function() {
  function injectAnimateLine2CSS(){
    if (document.querySelector('style[data-animate-line2-css]')) return;
    const st = document.createElement('style');
    st.setAttribute('data-animate-line2-css', 'true');
    st.textContent = `
/* Base setup for your target text */
.animate-line2 {
  position: relative;
  display: inline-block;
...
/* Absolute layer for the animated underline lines */
.lt-underline-layer {
  position: absolute;
  inset: 0;
  pointer-events: none;
...
/* Individual per-line underline bar */
.lt-line {
  position: absolute;
  left: 0;
  height: var(--lt-thickness, 2px);
  background: currentColor;
  transform-origin: left center;
  width: 0; /* collapsed until hover */
  transition: width 420ms cubic-bezier(.2, .7, .2, 1);
  will-change: width;
}

/* On hover, expand the underline(s) to full width */
.animate-line2.is-hover .lt-line {
  width: var(--lt-w, 0px);
}

/* Each word is measurable but wraps naturally */
.animate-line2 .lt-word {
  display: inline-block;
  white-space: normal;
}
`;
  ...
  // Utility: wrap words in spans so we can read per-line layout
  function wrapWords(el) {
    if (el.__lt_wrapped) return;
    ...
    el.appendChild(frag);
    el.__lt_wrapped = true;
  }

  // Group .lt-word spans into visual lines using offsetTop
  function computeLines(el) {
    const words = Array.from(el.querySelectorAll('.lt-word'));
    const lines = [];
    let current = [];
    let lastTop = null;
  ...
  // Create/update underline bars for each computed line
  function drawUnderlines(el) {
    const old = el.querySelector('.lt-underline-layer');
    if (old) old.remove();

    const layer = document.createElement('div');
    layer.className = 'lt-underline-layer';
    el.appendChild(layer);

    const cs = getComputedStyle(el);
    const fontSize = parseFloat(cs.fontSize) || 16;
    const weightNum = parseInt(cs.fontWeight, 10) || 400;

    // thickness heuristic: scales with size + weight
    const thickness = Math.max(
      1,
      Math.round(fontSize * (weightNum >= 700 ? 0.075 : weightNum >= 500 ? 0.06 : 0.045))
    );

    // offset from the bottom of the line box
    const underlineGap = Math.max(1, Math.round(fontSize * 0.08));

    // compute lines (after any layout changes)
    const lines = computeLines(el);
    ...
    lines.forEach((lineWords, i) => {
      ...
      const baseline = lineWords[0].offsetTop + lineWords[0].offsetHeight - underlineGap;

      const bar = document.createElement('div');
      bar.className = 'lt-line';

      // Place and size
      bar.style.setProperty('--lt-thickness', thickness + 'px');
      bar.style.top = baseline + 'px';
      bar.style.left = left + 'px';
      bar.style.setProperty('--lt-w', width + 'px');

      // Staggered reveal on hover (20ms * line index)
  ...
  function initAnimateLine2(){
    injectAnimateLine2CSS();

    const nodes = document.querySelectorAll('.animate-line2');
    nodes.forEach(el => {
      build(el);
      attachHover(el);
      // observe size changes to recompute
      if (!el.__lt_ro) {
        try {
          const ro = new ResizeObserver(() => drawUnderlines(el));
          ro.observe(el);
          el.__lt_ro = ro;
        } catch(_){}
      }
  ...
  // init on common lifecycle hooks (no changes to rest of file)
  document.addEventListener('DOMContentLoaded', initAnimateLine2);
  window.addEventListener('load', initAnimateLine2);

  // Optional: expose a manual refresh if content changes dynamically
  window.AnimateLine2Refresh = function() {
    document.querySelectorAll('.animate-line2').forEach(drawUnderlines);
  };
})();
// ======= FINE ANIMATE LINE 2 =======
