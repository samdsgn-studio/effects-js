...
.aline-line { /* each visual line */
  display: block; /* stack each visual line vertically */
  position: relative;
  vertical-align: baseline;
  white-space: normal;
  inline-size: max-content; /* shrink to the line's content width */
  max-inline-size: 100%; /* prevent overflow on narrow containers */
}
...
`;
    ...
    // Helper to rebuild the split when layout changes
    const rebuildNode = (root) => {
      if (!root) return;
      try {
        if (root._alineSplit && root._alineSplit.revert) {
          root._alineSplit.revert();
        }
      } catch(_) {}
      root._alineSplit = null;
      root.dataset.alineSplit = '';
      const textSpan = root.querySelector('.animate-line__text');
      if (textSpan) {
        splitPerLine(root);
      }
    ...
    // ResizeObserver to rebuild split on size changes
    try {
      if ('ResizeObserver' in window) {
        if (!window.__alineRO__) {
          window.__alineRO__ = new ResizeObserver((entries) => {
            const seen = new Set();
            entries.forEach(entry => {
              const target = entry.target;
              let root = null;
              if (target && target.matches && target.matches('.animate-line')) {
                root = target;
              } else if (target && target.closest) {
                root = target.closest('.animate-line');
              }
              if (root) seen.add(root);
            });
            seen.forEach(root => rebuildNode(root));
          });
        }
        document.querySelectorAll('.animate-line').forEach(el => {
          try { window.__alineRO__.observe(el); } catch(_) {}
        });
      }
    } catch(_) {}
    ...
    window.addEventListener('resize', () => {
      clearTimeout(__alineRT__);
      __alineRT__ = setTimeout(() => {
        document.querySelectorAll('.animate-line').forEach(rebuildNode);
      }, 150);
    }, { passive: true });
    ...
    try {
      const wrapNode = (node) => {
        ...
        if (node.matches && node.matches('.animate-line')) {
          ...
          splitPerLine(node);
          try { window.__alineRO__ && window.__alineRO__.observe(node); } catch(_) {}
        }
        node.querySelectorAll && node.querySelectorAll('.animate-line').forEach(el => {
          ...
          splitPerLine(el);
          try { window.__alineRO__ && window.__alineRO__.observe(el); } catch(_) {}
        });
      };
