...
.slide-in-bottom:hover .slide-in-bottom__inner,
.slide-in-bottom:focus-visible .slide-in-bottom__inner {
  transform: translateY(-100%);
}

.slide-in-bottom[data-copy] {
  cursor: pointer;
}
`;
  ...
  function build(el){
    ...
    // Optional: override mask spacer via CSS variable
    const maskPadAttr = el.getAttribute('data-mask-pad') || el.getAttribute('mask-pad');
    if (maskPadAttr && maskPadAttr.trim() !== '') {
      outer.style.setProperty('--split-hover-mask', maskPadAttr);
    }
...
// ======= INIZIO ANIMATE LINE 2 =======
(function(){
  function initAnimateLine2(){
    if (document.querySelector('style[data-animate-line2]')) return; // avoid double injection
    const style = document.createElement('style');
    style.setAttribute('data-animate-line2', 'true');
    style.textContent = `
:root {
  --aline2-speed: 0.35s;
  --aline2-ease: cubic-bezier(0.2, 0.7, 0.2, 1);
  --aline2-thickness: 2px;
}
...
/* Inner text wrapper (ensures underline sticks to content only) */
.animate-line2 > .animate-line2__text,
.animate-line2 > .animate-line2__text.animate-line2__text {
  display: inline !important;
  position: relative;
  white-space: normal; /* allow wrapping across multiple rows */
  width: auto !important;
...
/* Fallback single underline (used when no line-splitting available) */
.animate-line2 > .animate-line2__text::after {
  content: "";
  pointer-events: none;
  position: absolute;
  left: 50%;
  right: auto;
  bottom: 0;
  height: var(--aline2-thickness);
  width: 100%;
  transform: translateX(-50%) scaleX(0);     /* center-out */
  transform-origin: center;
  background-color: currentColor;
  transition: transform var(--aline2-speed) var(--aline2-ease);
}

/* Disable fallback when we have multiple visual lines */
.animate-line2.animate-line2--split > .animate-line2__text::after { display: none !important; }
...
/* Hover expands underline for all lines from center to edges */
.animate-line2:hover .aline2-line::after,
.animate-line2:focus-visible .aline2-line::after {
  transform: translateX(-50%) scaleX(1);
}

/* Also support hover on the container when only fallback is present */
.animate-line2:hover > .animate-line2__text::after,
.animate-line2:focus-visible > .animate-line2__text::after {
  transform: translateX(-50%) scaleX(1);
}
`;
    document.head.appendChild(style);

    // Ensure a dedicated inner text wrapper exists
    document.querySelectorAll('.animate-line2').forEach(node => {
      if (!node.querySelector('.animate-line2__text')) {
        const span = document.createElement('span');
        span.className = 'animate-line2__text';
        while (node.firstChild) {
          span.appendChild(node.firstChild);
        }
        node.appendChild(span);
    ...
    // Split per visual lines so multi-row texts underline each line
    const splitPerLine2 = (root) => {
      const textSpan = root.querySelector('.animate-line2__text');
      if (!textSpan || root.dataset.aline2Split === '1') return;
      if (!window.SplitText) return; // gracefully skip if plugin missing
      try { if (gsap.registerPlugin) gsap.registerPlugin(SplitText); } catch(_){}
      try {
        const s = SplitText.create(textSpan, { type: 'lines', linesClass: 'aline2-line' });
        if (s && s.lines && s.lines.length > 1) {
          root.classList.add('animate-line2--split');
        }
        root.dataset.aline2Split = '1';
      } catch(_){}
    };

    // Run on current nodes
    document.querySelectorAll('.animate-line2').forEach(splitPerLine2);

    // Observe DOM to auto-initialize future nodes
    try {
      const wrapNode = (node) => {
        if (!(node instanceof Element)) return;
        if (node.matches && node.matches('.animate-line2')) {
          if (!node.querySelector('.animate-line2__text')) {
            const span = document.createElement('span');
            span.className = 'animate-line2__text';
            while (node.firstChild) span.appendChild(node.firstChild);
        ...
        node.querySelectorAll && node.querySelectorAll('.animate-line2').forEach(el => {
          if (!el.querySelector('.animate-line2__text')) {
            const span = document.createElement('span');
            span.className = 'animate-line2__text';
            while (el.firstChild) span.appendChild(el.firstChild);
            el.appendChild(span);
      ...
      const mo = new MutationObserver((mutations) => {
        mutations.forEach(m => {
          m.addedNodes && m.addedNodes.forEach(wrapNode);
          if (m.type === 'attributes' && m.target && m.target.matches && m.target.matches('.animate-line2')) {
            wrapNode(m.target);
          }
        });
      });
  ...
  document.addEventListener('DOMContentLoaded', initAnimateLine2);
  window.addEventListener('load', initAnimateLine2);
})();
// ======= FINE ANIMATE LINE 2 =======
